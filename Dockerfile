...
RUN apt-get update &&  \
    apt-get install -y python3 python3-venv git python3-setuptools pipx &&  \
    apt-get clean &&  \
...
COPY package.json yarn.lock .yarnrc /tasmocompiler/
RUN cd /tasmocompiler && yarn install && yarn cache clean

# Then build the server
COPY public /tasmocompiler/public/
COPY server /tasmocompiler/server/
COPY src /tasmocompiler/src/
